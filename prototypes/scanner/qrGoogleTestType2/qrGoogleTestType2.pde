...
void draw()
{
  ...
  textSize(16);
  text(status, width/4, height/2); 
  text("Saving Account " + totalAccountsSaved + " of " + totalAccountNumb + "...", width/4, (height/2)+50);    //display the number of QR Codes currently saved
}
    ...
    for (int b = 0; b < 26; b++)
    {
      for (int c = 0; c < 26; c++)
      {
        for (int d = 0; d < 26; d++)
        {
          ...
          status = "Checking local image copy for account " + accountCode;

          if (fChecker.exists() == false) {    //check to see if the file exists or not
            status = "Local copy not found \nSaving QR Code for account " + accountCode; 
            ...
            status = "QR Code for account " + accountCode + " saved!";
          } else
          {
            status = "Local copy found, skipping account " + accountCode;
          }
          totalAccountsSaved++;
        }
      }
